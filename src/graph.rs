...
pub trait Graph<'q, NodeWeight: 'q, EdgeWeight: 'q, NodeRef: 'q, EdgeRef: 'q> {
    /**
     ...
     */
    fn adjacent_edges(&'q self, node: NodeRef) -> Result<Box<dyn Iterator<Item = EdgeRef> + 'q>>;

    /**
     * Checks whether two references refer to the same edge.
     */
    fn do_ref_same_edge(&'q self, edge1: EdgeRef, edge2: EdgeRef) -> Result<bool>;

    /**
     * Checks whether two references refer to the same node.
     */
    fn do_ref_same_node(&'q self, node1: NodeRef, node2: NodeRef) -> Result<bool>;
     ...
     */
    fn adjacent_nodes(&'q self, node: EdgeRef) -> Result<(NodeRef, NodeRef)>;

    /**
     * Retrieve weight from a node reference.
     */
    fn node_weight(&'q self, node: NodeRef) -> Result<&NodeWeight>;

    /**
     * Retrieve weight from an edge reference.
     */
    fn edge_weight(&'q self, edge: EdgeRef) -> Result<&EdgeWeight>;

    /**
     * Returns an Iterator over all node weights.
     */
    fn node_weights(&'q self) -> Box<dyn Iterator<Item = &'q NodeWeight> + 'q> {
     ...
     */
    fn edge_weights(&'q self) -> Box<dyn Iterator<Item = &'q EdgeWeight> + 'q> {
     ...
     */
    fn nodes(&'q self) -> Box<dyn Iterator<Item = NodeRef> + 'q>;

    /**
     * Returns an Iterator over all edges.
     */
    fn edges(&'q self) -> Box<dyn Iterator<Item = EdgeRef> + 'q>;
}
